...
- **检测维度**：系统内置多类检测能力，扫描时会根据目标 Agent 的能力进行智能探测，仅在存在对应能力时执行相应检测，以减少无效请求。主要检测类型包括：
  ...
  - **供应链安全类**：Agent 供应链安全检测（第三方依赖、插件、工具链中的恶意注入或后门等）
  - **代码执行安全类**：非预期代码执行检测（Agent 自主生成并执行未经授权的代码，导致沙箱逃逸或远程代码执行等）
  - **智能体间通信安全类**：多 Agent 间通信安全检测（Agent 间消息劫持、身份伪造、越权调用等）
  - **级联故障类**：级联故障检测（单个 Agent 故障在多 Agent 系统中引发连锁反应，导致大规模失效或安全边界突破等）
  - **人机信任利用类**：人机信任关系利用检测（通过社工手段欺骗用户授权、利用人类与 Agent 间的信任关系绕过安全机制等）

  > v4.5.1 新增 5 个 OWASP 检测技能（供应链安全、代码执行安全、智能体间通信安全、级联故障、人机信任利用）及 Web 数据外传检测，共计 10 个检测技能。前 5 项为默认检测项，后 5 项可通过 `--skills` 参数按需启用。
